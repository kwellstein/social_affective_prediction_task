...
class EyeLinkCoreGraphicsPsychoPy(pylink.EyeLinkCustomDisplay):
    def __init__(self, tracker, win):
        ...
        self.title = visual.TextStim(self.display,'dfdf', height=self.msgHeight, color=[1,1,1],
                                     pos = (0,-self.size[1]/2-self.msgHeight), wrapWidth=self.w, units='pix')
                                           
        self.calibInst = visual.TextStim(self.display,'as', anchorHoriz='left',anchorVert ='top', height=self.msgHeight, color=[1,1,1],pos = (-self.w/2.0, self.h/2.0), units='pix')                          
        self.calibInst.text='''
