...
win = visual.Window((scnWidth, scnHeight), fullscr=False, monitor=mon, color=[0,0,0], units='pix')
"""
win = visual.Window((scnWidth, scnHeight), fullscr=False, screen=0, color=[0,0,0], units='pix')
...
def runTrial(pars, trialCounter):
    ...
    if trialCounter > 1:
        print("Within trial not 1: sending message: ",'TRIALID ' + str(trialCounter))
        tk.sendMessage('TRIALID ' + str(trialCounter))
    
    # show the image indefinitely or press a key to terminate a trial
    if trialCounter == 1:
        terminate = False
        print("Within trial 1: Awaiting a key")
        while not terminate:
			#need to define which key to use here so that it corresponds to the scanner key that's used
    ...
    print("Within trial: Movie start")
    
    #JAYSON has replaced the real movie-playing code commented below, with these 3 surrogate lines
    for i in range(5):
        tk.sendMessage('MOVIE_FRAME ' + str(i))
        core.wait(0.4)
    ...
    while not terminate:
        ...
        if nextFrameTimeStamp > currentFrameTimeStamp:
            frameNum += 1
            tk.sendMessage('MOVIE_FRAME ' + str(frameNum)) # this message marks the time 0 of a trial, can also send the "DISPLAY_SCREEN" message here
    ...
    """
    print("Within trial: Movie end")   
    
    
    # clear the subject display
    win.color=[0,0,0]
    win.flip()
    tk.sendMessage('blank screen')
...
msg = visual.TextStim(win, text = '', color = 'black', units = 'pix')
msg.draw(); win.flip()
print('Press Spacebar to continue')
event.waitKeys()

# set up the camera and calibrate the tracker at the beginning of each block
print('Setup started. Press Enter to mirror to BOLD screen')
tk.doTrackerSetup() #in Eyelink.py within the Eyelink library
print('setup done')

blocks = 2

for blockCounter in range(blocks):
    print("blockstart",blockCounter)
	...
	# drift check (JAYSON HAS COMMENTED THIS OUT)
    """
    try:
        print("Trying drift check")
        err = tk.doDriftCorrect(win.size[0]/2, win.size[1]/2,1,1)
    except:
        print("Drift check exception: Returned to doTrackerSetup")
        tk.doTrackerSetup()    
	"""
    ...
    trialCounter = 1
    for t in testList: 
        print("trialstart",trialCounter)
        runTrial(t,trialCounter)
        print("trialend",trialCounter)
        trialCounter = trialCounter + 1
	
    print("blockend",blockCounter)
    tk.sendMessage('End Block' + str(blockCounter + 1))
...
msg.draw(); win.flip()
tk.setOfflineMode()
print("Going to Receive DataFile")
