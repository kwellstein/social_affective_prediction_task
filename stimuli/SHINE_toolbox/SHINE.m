%% JJ has added code wherever it says 'ADDED CODE'. JJ runs this by editing code lines below, then start program with command 'SHINE' then selecting 'default'.
...
%ADDED CODE (code corresponding to this is near end of script)
target_lum=[128,42];
lumStart_whole=true; %At start, do lumMatch(target_lum). 
lumStart_foreground=false; %At start, do lumMatch(foreground, background)
lumEnd=true; %At end, do lumMatch(target_lum)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% If desired, the default values can be changed here:

it = 10;           % number of iterations (default = 1)

wholeIm = 2;      % 1 = whole image (default)
                  % 2 = figure-ground separated (input images as templates)
                  % 3 = figure-ground separated (based on templates)

mode = 5;         % 1 = lumMatch only
                  % 2 = histMatch only
                  % 3 = sfMatch only
                  % 4 = specMatch only
                  % 5 = histMatch & sfMatch
...
disp(['SETTINGS: iterations: ', num2str(it),' | wholeIm: ', num2str(wholeIm), ' | mode: ',...
    num2str(mode), ' | background :', num2str(background),' | rescaling :', num2str(rescaling), ' | optim: ', num2str(optim)]);
...
%ADDED CODE to display initial std
S=0;
for im = 1:numim
    S = S + std2(images{im});
end
S = S/numim;
sprintf('Initial std: %f',S)

%ADDED code to start with lumMatch for fore/backgrounds
if lumStart_foreground
    images = lumMatch(images,mask_fgr);
    images = lumMatch(images,mask_bgr);
end
if lumStart_whole
    images=lumMatch(images,[],target_lum);
end
...
%ADDED CODE to end with lummatch (forcing mean and std)
if lumEnd
    images = lumMatch(images,[],target_lum); %default target_lum 128,42 
end
...
for im = 1:numim
    if nargout == 0
        if nargin > 0
            ...
            imwrite(images{im},fullfile(output_folder,strcat(num2str(im),'.tif')));
        else
            %imwrite(images{im},fullfile(output_folder,strcat('SHINEd_',imname{im})));
            imwrite(images{im},fullfile(output_folder,strcat('',imname{im})));
        end
    end
...
save(fullfile(output_folder,diaryName),'diaryName');
diary('off')
