function tapas_hgf_plotTraj_mod(r)
...
for j = 1:l
    ...
    ylabel(['\mu_', num2str(l-j+1)],'FontSize',14);
end
...
hold all;
plot(0, r.p_prc.mu_0(1), 'or', 'LineWidth', 1); % prior
plot(ts(2:end), r.u(:,1), '.', 'Color', [0 0.6 0],'Markersize',10); % inputs
plot(ts(2:end), r.u(:,1), 'Color', [0 0.6 0], 'LineWidth', 2)
if ~isempty(find(strcmp(fieldnames(r),'y'))) && ~isempty(r.y)
    plot(ts(2:end), r.y(:,1), 'o', 'Color', [1 0.7 0],'Markersize',6); % responses
    plot(ts(2:end), r.y(:,1), 'Color', [1 0.7 0], 'LineWidth', 2)
    ...
    title(['Input u (green) and posterior expectation of x_1 ', ...
           '(black line) for \rho=', num2str(r.p_prc.rho), ', \kappa=', ...
           num2str(r.p_prc.ka), ', \omega=', num2str(r.p_prc.om)], ...
          'FontWeight', 'bold');
    ylabel('u, \mu_1','FontSize',14);
end
ylim([-0.1 1.1]);
xlim([0 ts(end)]);
xlabel({'Trial number', ' '},'FontSize',14); % A hack to get the relative subplot sizes right
hold off;
